...
/// @dev Liquidity pool for a token pair
    struct TkLiquidityPool {      /// @dev Blueprint describing a token-pair liquidity pool
...
*/
    function _pairTkHash(address tokenX, address tokenY)
        internal
        pure
        returns (bytes32)
    {
        ...
        )
    {
        ...
        if (pool.totalTkLiquidityPool == 0) {
            ...
            if (optimalB <= amountBDesired) {
                ...
                uint256 optimalA = (amountBDesired * pool.TkDepositA) /
                    pool.TkDepositB;
...
*/
    ...
    function removeLiquidity(
        ...
        (amountA, amountB) = _removeLiquidityCore(
            token0,
            token1,
            liquidity,
            min0,
...
*/
        ...
        returns (uint256 price)
    {
        ...
        TkLiquidityPool storage pool = pairTk[poolId];
        require(
            pool.TkDepositA > 0 && pool.TkDepositB > 0,
            "Cannot quote price: pool has no liquidity."
        );
